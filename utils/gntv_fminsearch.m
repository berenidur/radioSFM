function params_block_reg = gntv_fminsearch(params_init, bscblock, f, cfg)
...
%   params_block_reg : M x N x P final parameter maps
    ...
    if ~isfield(cfg,'tv_eps') || isempty(cfg.tv_eps)
        cfg.tv_eps = 1e-6;     % small epsilon for smoothed TV
    end
    ...
    fun = @(x) residual_global_tv_prior(x, params_init, channels, ...
                                        bscblock, f, mask_valid, ...
                                        idx_valid, lambda, cfg.tv_eps);

    % --- Call lsqnonlin (Levenberg–Marquardt ~ Gauss–Newton) ---
    costfun = @(x) cost_global_tv_prior(x, params_init, channels, ...
                                    bscblock, f, mask_valid, ...
                                    idx_valid, lambda, cfg.tv_eps);

    x_opt = fminsearch(costfun, x0, cfg.fmin_opts);
...
function J = cost_global_tv_prior(x, params_init, channels, ...
                                  bscblock, f, mask_valid, ...
                                  idx_valid, lambda, tv_eps)
    % First compute the residual vector using your existing function:
    r = residual_global_tv_prior(x, params_init, channels, ...
                                 bscblock, f, mask_valid, ...
                                 idx_valid, lambda, tv_eps);

    % fminsearch requires a SCALAR objective
    J = sum(r.^2);     % equivalent to lsqnonlin minimization target
end
